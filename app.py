...
REVIEW_SYSTEM_PROMPT = """你是一名专业的AI视频质量评审员。观看提供的视频片段，对照分镜提示词，从以下5个维度进行评审。
...
输出严格的JSON格式，不要包含任何额外文字或Markdown标记：
{
  ...
  "temporal_consistency": {"score": 1-10, "comment": "时域一致性：相邻帧间是否闪烁、抖动、形变，运动轨迹是否符合物理规律"},
...
def _generate_video_backend(prompt, image_url, video_model, api_key, resolution="720P", duration=5, char_image_url=None, negative_prompt=""):
    """后台生成视频（同步等待完成）
    char_image_url: r2v模式下的角色参考图（/images/xxx 或本地路径）。
    r2v要求: reference_images + first_frame_url 必须同时提供，否则报错 Only first frame provided
    negative_prompt: 反向提示词（wanx模式使用）
    返回: {'success': True/False, 'url': str, 'filename': str, 'video_url': str/None, 'error': str}
    """
    try:
        ...
        while waited < max_wait:
            ...
            if status_result.get('status') == 'SUCCEEDED':
                cdn_url = status_result.get('video_url', '')
                if cdn_url:
                    # 下载视频到本地（download_video内部会检查文件已存在则跳过）
                    try:
                        local_path, filename = download_video(cdn_url, vid_task_id)
                        return {
                            'success': True,
                            'filename': filename,
                            'url': f'/videos/{filename}',
                            'video_url': cdn_url  # CDN URL 供 VLM 评审使用
                        }
                    except Exception as de:
                        print(f"[生成视频] 下载失败: {de}")
                        return {'success': False, 'error': f'视频下载失败: {str(de)}'}
                else:
                    return {'success': False, 'error': '视频生成完成但无URL'}
...
def _review_video_backend(video_url, prompt, api_key):
    """后台评审单个视频（使用专业REVIEW_SYSTEM_PROMPT，含时域一致性维度）"""
    try:
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {api_key or QWEN_API_KEY}"
        ...
        if content:
            ...
            if json_match:
                review = json.loads(json_match.group())
                # 兼容历史返回中仍用improvements字段的情况
                if 'improvements' in review and ('issues' not in review or not review['issues']):
                    review['suggestions'] = review.get('suggestions', []) or review['improvements']
                    review['issues'] = review.get('issues', []) or [f'需改进: {imp}' for imp in review['improvements'][:3]]
                return review
        
        return {'overall_score': 0, 'comment': '评审失败', 'issues': [], 'suggestions': []}
    except Exception as e:
        return {'overall_score': 0, 'comment': f'评审异常: {str(e)}', 'issues': [], 'suggestions': []}
...
def _review_global_image_consistency(scenes, scene_versions, api_key):
    """全局检查所有分镜图片的一致性（人物、风格、场景连贯性）
    
    收集每个分镜当前最佳图片，调用VLM一次性审查所有图片间的视觉一致性问题。
    
    Returns:
        dict: {
            'overall_consistency_score': int,
            'issues': [{'scene_index': i, 'type': str, 'description': str, 'severity': 'high|medium|low'}],
            'comment': str
        }
    """
    try:
        ...
        for i, scene in enumerate(scenes):
            sv = scene_versions.get(str(i), {})
            images = sv.get('images', [])
            if not images:
                continue
            best = max(images,
                key=lambda x: (x.get('image_review') or {}).get('overall_score', 0)
                if x.get('image_review') else 0)
            img_data = best.get('image')
            if not img_data:
                continue
            ...
            # 将本地URL转为base64（复用_review_image_backend的逻辑）
            vlm_url = img_url
            if img_url.startswith('/images/'):
                ...
                if os.path.exists(local_path):
                    try:
                        ...
                        b64 = base64.b64encode(buf.getvalue()).decode()
                        vlm_url = f"data:image/jpeg;base64,{b64}"
                    except Exception as ce:
                        print(f"[全局一致性评审] 图片转换失败: {ce}")
                        vlm_url = f"http://localhost:5000{img_url}"
            
            content_items.append({"type": "image_url", "image_url": {"url": vlm_url}})
            scene_summaries.append(f"分镜{i+1}: {scene.get('scene_title', '')} - {scene.get('scene_desc', '')[:80]}")
        
        if not content_items:
            print('[全局一致性评审] 无图片可评审')
            return {'overall_consistency_score': 0, 'issues': [], 'comment': '无图片可评审'}
        
        story_summary = ' '.join([s.get('scene_desc', '') for s in scenes])[:500]
        review_text = (
            f"请评审以下所有分镜的图片是否具备视觉一致性。\n\n"
            f"故事：{story_summary}\n\n"
            f"分镜顺序：\n"
            f"{'\n'.join(scene_summaries)}\n\n"
            f"评审维度：\n"
            f"1. 人物一致性：同一角色在不同分镜中外貌、服装、体型是否一致\n"
            f"2. 风格一致性：所有图片的画面风格、色调、光影是否统一\n"
            f"3. 场景连贯性：环境元素、道具在不同场景间是否前后呼应\n"
            f"4. 叙事流畅性：画面组合是否构成完整的视觉叙事\n\n"
            f"严格输出JSON，不要包含任何额外文字或Markdown标记：\n"
            f"{{\n"
            "  \"overall_consistency_score\": 1-10的整数,\n"
            "  \"issues\": [\n"
            "    {\n"
            "      \"scene_index\": 分镜索引(0开始),\n"
            "      \"type\": \"character_appearance|style|scene|narrative\",\n"
            "      \"description\": \"问题描述（中文）\",\n"
            "      \"severity\": \"high|medium|low\"\n"
            "    }\n"
            "  ],\n"
            "  \"comment\": \"总体评价（中文，60字内）\"\n"
            "}\n\n"
            f"只有完全一致才给10分，有明显不一致就给低分并在issues中详细说明。"
        )
        
        content_items.append({"type": "text", "text": review_text})
        
        payload = {
            "model": "qwen-vl-max",
            "messages": [
                {"role": "system", "content": "你是一名专业的电影视觉一致性评审员，擅长发现多张图片之间的视觉不连贯问题。"},
                {"role": "user", "content": content_items}
            ]
        }
        
        resp = requests.post(QWEN_API_URL, json=payload, headers=headers, timeout=180)
        result = resp.json()
        ...
        if content:
            ...
            if json_match:
                review = json.loads(json_match.group())
                _add_task_log(None, f'全局一致性评审完成，得分{review.get("overall_consistency_score", 0)}，发现{len(review.get("issues", []))}个问题')
                return review
        
        print(f'[全局一致性评审] JSON解析失败，原始: {content[:500] if content else "None"}')
        return {'overall_consistency_score': 0, 'issues': [], 'comment': '评审解析失败'}
    except Exception as e:
        print(f'[全局一致性评审] 异常: {e}')
        return {'overall_consistency_score': 0, 'issues': [], 'comment': f'评审异常: {str(e)}'}


def _refine_prompt_by_review(original_prompt, review_result, prompt_type, api_key, global_feedback=None):
    """根据评审结果调用LLM修正提示词（自优化闭环）
    支持传入global_feedback列表，将历史反馈中的规律性问题一并带入修正。
    """
    try:
        ...
        # 构建历史反馈摘要
        feedback_summary = ''
        if global_feedback and isinstance(global_feedback, list) and len(global_feedback) > 0:
            pattern_issues = set()
            for fb in global_feedback[-5:]:  # 取最近5条
                if isinstance(fb, dict):
                    correction = fb.get('correction', {})
                    summary = correction.get('correction_summary', '') if isinstance(correction, dict) else ''
                    if summary:
                        feedback_summary += f"  - {summary}\n"
                    issues_list = fb.get('issues', [])
                    for iss in (issues_list if isinstance(issues_list, list) else []):
                        pattern_issues.add(iss)
            if pattern_issues:
                feedback_summary += f"历史反复出现的问题：{'/'.join(list(pattern_issues)[:3])}\n"
            if feedback_summary:
                feedback_summary = f"\n=== 历史反馈摘要（共{len(global_feedback)}条）===\n{feedback_summary}"
...
def run_auto_workflow(task_id, params):
    """后台线程执行全自动工作流（版本追踪版）
    四阶段流程：
      阶段1-2: 分镜生成 + 分镜评审（本地变量进化，不污染全局）
      阶段3: 逐镜图片生成（多版本+自优化+全局反馈）
      等待用户确认: images_done 状态，用户可查看/选版/设角色图
      阶段4: 逐镜视频生成（多版本+自优化+全局反馈+CDN评审URL）
      等待用户确认: videos_done 状态，用户可预览视频
      最终汇总: 自动生成 final_review
    """
    ...
    try:
        ...
        # ====== 阶段2: 分镜评审 ======
        # 使用本地变量进化提示词，不污染全局 CURRENT_SCENE_PROMPT
        scene_prompt_template = SCENE_SYSTEM_PROMPT
        ...
        for review_cycle in range(max_versions):
            ...
            if review_cycle < max_versions - 1:
                ...
                try:
                    current_prompt = scene_prompt_template
                    evolve_payload = {
                        "model": "qwen-max",
                        "messages": [
                            {"role": "system", "content": SCENE_EVOLVE_SYSTEM_PROMPT},
                            {"role": "user", "content": (
                                ...
                                f"详细评审：\n{json.dumps(review_result.get('scenes', []), ensure_ascii=False, indent=2)[:2000]}\n\n"
                                f"请分析各分镜的薄弱维度，有针对性地优化分镜生成标准。（请输出完整系统提示词）"
                            )}
                        ],
                    ...
                    if 'json' not in new_prompt.lower():
                        new_prompt += "\n\n请始终以合法的JSON格式输出。"
                    scene_prompt_template = new_prompt
        ...
        for i, scene in enumerate(scenes):
            ...
            for v in range(max_versions):
                ...
                if check_pause_requested(task_id):
                    _add_task_log(task_id, '任务暂停', 'warn')
                    _update_task_status(task_id, status='paused', current_action='已暂停')
                    wait_for_user_confirmation(task_id, 'image', {})
                    _update_task_status(task_id, status='running', current_action=f'分镜{i+1} 图片生成 (版本{version_num})')
                ...
                with auto_tasks_lock:
                    gf = auto_tasks.get(task_id, {}).get('global_feedback', [])
                old_prompt = image_prompt
                image_prompt = _refine_prompt_by_review(
                    image_prompt, img_review, 'image', api_key,
                    global_feedback=gf
                )
                _add_task_log(task_id, f'分镜{i+1} 图片提示词已修正')
                # 沉淀全局反馈
                if image_prompt != old_prompt:
                    with auto_tasks_lock:
                        gf_list = auto_tasks.get(task_id, {}).get('global_feedback', [])
                        gf_list.append({
                            'scene_index': i, 'type': 'image',
                            'version': version_num,
                            'issues': img_review.get('issues', []),
                            'suggestions': img_review.get('suggestions', []),
                            'score': img_score,
                            'correction_summary': f'分镜{i+1} 图片v{version_num} 得分{img_score}，已修正提示词',
                            'timestamp': datetime.now().isoformat()
                        })
                        auto_tasks[task_id]['global_feedback'] = gf_list
        
        # ====== 全局图片一致性评审 ======
        _add_task_log(task_id, '阶段3.5: 全局图片一致性评审')
        _update_task_status(task_id, stage='image_global_review', total_progress=50,
                            current_action='正在全局检查图片一致性...')
        
        for global_cycle in range(max_versions):
            _add_task_log(task_id, f'全局一致性评审 (第{global_cycle+1}轮)')
            
            # 收集每镜最佳图片并评审
            consistency_review = _review_global_image_consistency(scenes, scene_versions, api_key)
            consistency_score = consistency_review.get('overall_consistency_score', 0)
            consistency_issues = consistency_review.get('issues', [])
            
            if not consistency_issues:
                _add_task_log(task_id,
                    f'全局图片一致性检查通过（得分{consistency_score}/10），共{len(scenes)}个分镜', 'success')
                break
            
            _add_task_log(task_id, f'全局一致性得分{consistency_score}/10，发现{len(consistency_issues)}个问题', 'warn')
            for iss in consistency_issues:
                _add_task_log(task_id,
                    f'  分镜{iss.get("scene_index", 0)+1} [{iss.get("severity", "medium")}]: {iss.get("description", "")}')
            
            # 按 severity 排序（high 优先重生成）
            sorted_issues = sorted(consistency_issues,
                key=lambda x: {'high': 0, 'medium': 1, 'low': 2}.get(x.get('severity', 'medium'), 1))
            
            regenerated_count = 0
            for iss in sorted_issues:
                scene_idx = iss.get('scene_index')
                if scene_idx is None or scene_idx < 0 or scene_idx >= len(scenes):
                    continue
                
                _add_task_log(task_id, f'重新生成分镜{scene_idx+1} 图片...')
                scene = scenes[scene_idx]
                img_prompt = scene.get('image_prompt', '')
                if not img_prompt:
                    _add_task_log(task_id, f'分镜{scene_idx+1} 无image_prompt，跳过', 'warn')
                    continue
                
                # 获取该分镜的人物参考图
                with auto_tasks_lock:
                    task_data = auto_tasks.get(task_id, {})
                    scene_chars = task_data.get('scene_characters', {}).get(str(scene_idx), [])
                ref_paths = []
                for char_id in scene_chars:
                    char_obj = next((c for c in characters if c['id'] == char_id), None)
                    if char_obj:
                ...
                # 重新生成
                new_img_result = _generate_image_backend(
                    img_prompt, image_model, api_key,
                    reference_images=ref_paths,
                    negative_prompt=negative_prompt
                )
                
                new_version_num = len(scene_versions[str(scene_idx)]['images']) + 1
                new_version_obj = {
                    'version': new_version_num,
                    'image': new_img_result if new_img_result.get('success') else None,
                    'image_error': new_img_result.get('error') if not new_img_result.get('success') else None,
                    'image_review': None,
                    'manual_review': None,
                    'image_passed': False,
                    'image_prompt_used': img_prompt,
                    'global_review_issue': iss.get('description', ''),  # 标记为全局评审重生成
                }
                
                if not new_img_result.get('success'):
                    _add_task_log(task_id, f'分镜{scene_idx+1} 重生成失败: {new_img_result.get("error")}', 'error')
                    scene_versions[str(scene_idx)]['images'].append(new_version_obj)
                    continue
                
                _add_task_log(task_id, f'分镜{scene_idx+1} 图片重生成完成（版本{new_version_num}）', 'success')
                
                # 对新图片做独立评审
                new_img_review = _review_image_backend(
                    new_img_result.get('url', ''),
                    scene.get('scene_desc', ''),
                    api_key
                )
                new_img_score = new_img_review.get('overall_score', 0)
                new_version_obj['image_review'] = new_img_review
                new_version_obj['image_passed'] = (new_img_score >= score_threshold)
                scene_versions[str(scene_idx)]['images'].append(new_version_obj)
                
                # 更新该分镜的最佳版本选择
                best_score = max(
                    (vv.get('image_review', {}) or {}).get('overall_score', 0)
                    if vv.get('image_review') else 0
                    for vv in scene_versions[str(scene_idx)]['images']
                )
                best_idx = next(
                    j for j, vv in enumerate(scene_versions[str(scene_idx)]['images'])
                    if (vv.get('image_review', {}) or {}).get('overall_score', 0) == best_score
                )
                scene_versions[str(scene_idx)]['selected_image_idx'] = best_idx
                _add_task_log(task_id, f'分镜{scene_idx+1} 新图片评审得分{new_img_score}/10')
                regenerated_count += 1
            
            _update_task_status(task_id, scene_versions=scene_versions)
            _add_task_log(task_id, f'本轮全局一致性评审共重生成{regenerated_count}个分镜的图片')
            
            if global_cycle == max_versions - 1:
                _add_task_log(task_id, f'已达最大全局评审轮次，将使用当前最佳版本', 'warn')
        ...
        for i, scene in enumerate(scenes):
            ...
            # 从分镜数据解析视频时长（如'6秒，均匀平稳'→6）
            import re as _re
            duration_match = _re.search(r'(\d+)', scene.get('duration', '5'))
            vid_duration = int(duration_match.group(1)) if duration_match else 5
            vid_duration = max(2, min(vid_duration, 15))  # 限制在 2-15 秒
            ...
            for v in range(max_versions):
                ...
                if check_pause_requested(task_id):
                    _add_task_log(task_id, '任务暂停', 'warn')
                    _update_task_status(task_id, status='paused', current_action='已暂停')
                    wait_for_user_confirmation(task_id, 'video', {})
                    _update_task_status(task_id, status='running', current_action=f'分镜{i+1} 视频生成 (版本{version_num})')
                ...
                _add_task_log(task_id, f'分镜{i+1}: 生成视频 (分辨率{video_resolution}, 时长{vid_duration}秒)')
                ...
                # 视频评审（优先使用CDN URL，避免localhost硬编码）
                _add_task_log(task_id, f'分镜{i+1}: 视频评审')
                vid_review_url = (vid_result.get('video_url', '') or
                ...
                with auto_tasks_lock:
                    gf = auto_tasks.get(task_id, {}).get('global_feedback', [])
                video_prompt = _refine_prompt_by_review(
                    video_prompt, vid_review, 'video', api_key,
                    global_feedback=gf
                )
                ...
                with auto_tasks_lock:
                    gf_list = auto_tasks.get(task_id, {}).get('global_feedback', [])
                    gf_list.append({
                        'scene_index': i, 'type': 'video',
                        'version': version_num,
                        'issues': vid_review.get('issues', []),
                        'suggestions': vid_review.get('suggestions', []),
                        'score': vid_score,
                        'correction_summary': f'分镜{i+1} 视频v{version_num} 得分{vid_score}，已修正提示词',
                        'timestamp': datetime.now().isoformat()
                    })
                    auto_tasks[task_id]['global_feedback'] = gf_list
        ...
        _update_task_status(task_id,
            stage='videos_done',
            total_progress=90,
            current_action='视频生成完成！请预览各版本，确认后点击"完成"',
            scene_versions=scene_versions
        )
        _add_task_log(task_id,
            '所有分镜视频生成完成！请预览各版本，'
            '如需可手动调整，确认后点击"完成"。', 'success')
        
        if not wait_for_user_confirmation(task_id, 'videos_done', {}):
            return  # 用户停止了任务
        
        # ====== 最终汇总评审 ======
        _update_task_status(task_id, stage='final_review',
                            total_progress=95, current_action='正在生成最终评审汇总...')
        _add_task_log(task_id, '生成最终评审汇总...')
        try:
            # 收集所有视频评审结果
            scene_reviews = []
            overall_scores = []
            for i, scene in enumerate(scenes):
                sv = scene_versions.get(str(i), {})
                videos = sv.get('videos', [])
                if videos:
                    best_vid = max(videos,
                        key=lambda vv: (vv.get('video_review', {}) or {}).get('overall_score', 0)
                        if vv.get('video_review') else 0)
                    review = (best_vid.get('video_review') or {})
                    ...
                    overall_scores.append(review.get('overall_score', 0))
            
            final_review = {
                'overall_score': (sum(overall_scores) / len(overall_scores))
                                 if overall_scores else 0,
                'comment': f'共 {len(scenes)} 个分镜，{len(overall_scores)} 个生成视频',
                'scene_reviews': scene_reviews,
            }
            _add_task_log(task_id, f'最终汇总得分: {final_review["overall_score"]:.1f}/10', 'success')
        except Exception as fre:
            print(f"[最终评审] 生成失败: {fre}")
            final_review = {'overall_score': 0, 'comment': '汇总异常', 'scene_reviews': []}
